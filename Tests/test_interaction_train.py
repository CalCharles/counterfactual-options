...
def test_passive_model():
    ...
    data = read_obj_dumps(pth="data/unit_test/interaction_train/", i=-1, rng = 3000, filename='object_dumps.txt')
    ...
    model_name = "Paddle" + "->"+ "Ball"
    model_args = default_model_args(args.predict_dynamics, args.policy_type)
    model_args['name'] = model_name
    model_args['gamma'] = environment_model.create_entity_selector(["Ball"])
    model_args['delta'] = environment_model.create_entity_selector(["Ball"])
    model_args['object_dim'] = environment_model.object_sizes["Ball"]
    model_args['output_dim'] = environment_model.object_sizes["Ball"]
    model_args['first_obj_dim'] = environment_model.object_sizes["Ball"]
    nout = environment_model.object_sizes["Ball"] * environment_model.object_num["Ball"]
    nin = environment_model.object_sizes["Ball"] * environment_model.object_num["Ball"] + nout
    ...
    target_passive = load_from_pickle("data/unit_test/interaction_test/passive_output.pkl") # contains active and passive outputs
    target_model = torch.load("data/unit_test/interaction_test/target_interaction_model.pt")
    ...
    trace = model.generate_interaction_trace(rollouts, ["Paddle"], ['Ball'])
    outputs = model._train_interaction(rollouts, args, batchvals, trace, interaction_optimizer)
    target_interaction = load_from_pickle("data/unit_test/interaction_test/interaction_output.pkl")
    for o, ti in zip(outputs, target_interaction):
    	print("interaction output difference (should be zero):", np.linalg.norm(pytorch_model.unwrap(o) - pytorch_model.unwrap(ti)))
    for p, ti in zip(model.interaction_model.get_parameters(), target_model.interaction_model.get_parameters()):
    	print("interaction weight difference: ", np.linalg.norm(pytorch_model.unwrap(p) - pytorch_model.unwrap(ti)))
    ...
    trace, weights, use_weights, passive_error_all, interaction_schedule,
    active_optimizer, passive_optimizer, interaction_optimizer)        
    target_passive = load_from_pickle("data/unit_test/interaction_test/active_output.pkl") # contains active and passive outputs
    for o, tp in zip(outputs, target_passive):
