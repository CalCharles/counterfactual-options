...
class Sampler():
    def __init__(self, **kwargs):
        ...
        self.lower_cfs = np.array([i for i in [cfs.feature_range[0] for cfs in cfselectors]])
        self.upper_cfs = np.array([i for i in [cfs.feature_range[1] for cfs in cfselectors]])
        self.len_cfs = np.array([j-i for i,j in [tuple(cfs.feature_range) for cfs in cfselectors]])

    def sample(self, states):
        # takes in states of size num_sample x state_size, and return samples 
        return

    def weighted_samples(self, states, weights):
...
class GaussianCenteredSampling():
    def __init__(self, **kwargs):
