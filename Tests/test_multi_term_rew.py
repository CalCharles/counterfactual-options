...
def test_multi_terminate_reward():
    args = ObjDict()
    args.seed = 0
    ...
    dataset_model = load_hypothesis_model("data/unit_test/ball_block/")
    dataset_model.environment_model = environment_model
    dataset_model.cpu()

    --temporal-extend 300 --sac-alpha .2 --lookahead 4 --print-test --use-interact --max-hindsight 12 --param-interaction --use-termination --param-first --max-distance-epsilon 6 --record-rollouts data/block --save-graph data/block_graph_model --save-interval 100 > logs/train_block_model.txt
    ...
    args.name = "Block"
    args.object = "Block"
    args.interaction_probability = 1
    args.param_interaction = True
    args.true_reward_lambda = 0
    args.parameterized_lambda = 100
    args.interaction_lambda = 0
    args.reward_constant = -.01
    args.time_cutoff = 300
    args.use_termination = True
    args.terminal_type = "proxist"
    args.reward_type = "proxist"
    ...
    args.epsilon_min = 1
    args.epsilon_close = 0.5
    args.epsilon_close_schedule = 0
    args.interaction_prediction = .2
    args.state_extractor = TestExtractor(target="Block")
    terminate_reward = TerminateReward(args)

    models = ObjDict()
    models.sampler = TestSamplerBlock()
    ...
    batches = list()
    for i in range(3):
        collect_result = train_collector.collect(n_step=100)
