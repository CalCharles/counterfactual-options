...
TODOs: move the epsilons into the policy


def _collect_test_trials(args, test_collector, i, total_steps, test_perf, suc, trials, action_record=None, random=False, option=None):
    '''
    '''
    ...
    for j in range(trials):
        ...
        suc.append(float(result["step_terminated"]))
    if random:
    ...
    return mean_perf, mean_suc, mean_hit, test_perf, 
...
def trainRL(args, train_collector, test_collector, environment, environment_model, option, names, graph):
    ...
    initial_perf, initial_suc, initial_hit = _collect_test_trials(args, test_collector, 0, 0, initial_perf, initial_suc, args.pretest_trials,  random=True, action_record=action_record, option=option)
