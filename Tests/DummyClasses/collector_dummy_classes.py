...
class TestOption():
    ...
    def extended_action_sample(self, batch, state_chain, term_chain, ext_terms, random=False):
        print("action call", batch.target, batch.param)
        if np.random.randint(5) == 0 and self.driving:
    ...
    def terminate_reward_chain(self, full_state, next_full_state, param, chain, mask, mask_chain, environment_model=None):
        ...
        print("term", termination, reward, inter, full_state["factored_state"]["Paddle"], next_full_state["factored_state"]["Paddle"], param)
...
class TestOptionBlock(TestOption):
    def extended_action_sample(self, batch, state_chain, term_chain, ext_terms, random=False):
        if np.random.randint(2) == 0 or (self.driving and np.random.randint(10) > 2):
            array_factored(full_state['factored_state'])
            p = batch.full_state['factored_state']['Paddle'].squeeze()
            t = batch.full_state['factored_state']['Ball'].squeeze()
            target = (p - t)[1]
            if abs(target) < 1:
                act = 0
            elif target > 0:
    ...
    def terminate_reward_chain(self, full_state, next_full_state, param, chain, mask, mask_chain, environment_model=None):
        ...
        array_factored(next_full_state['factored_state'])
        termination, reward, inter, time_cutoff = self.terminate_reward.check(full_state, next_full_state, param, mask)
        last_termination = full_state['factored_state']["Ball"].squeeze()[2] > 0 and full_state['factored_state']["Ball"].squeeze()[2] < 0
        ext_term = self.temporal_extension_manager.get_extension(termination, last_termination)
        done = self.done_model.check(termination, np.array(next_full_state['factored_state']['Done']).squeeze())


        rewards, terminations, ext_term = [0] + [reward], [last_termination] + [termination], list() + [last_termination]
...
class TestSamplerLocal():
    def get_param(self, full_state, terminate):
        if terminate:
            states = full_state['factored_state']
            shape = np.array(states['Action']).shape
            self.mask = np.array([0,1,0,0,0])
            self.param = np.array([0, full_state['factored_state']["Paddle"][1] + np.random.rand() * 2 * ((np.random.randint(3) - 1)), 0, 0, 0])
            print("new sample", self.param)
        return self.param, self.mask, terminate
...
class TestTemporalExtensionManager():
    ...
    def get_extension(self, terminate, ext_term):
        return 1# terminate + ((np.random.randint(2) == 1) * (np.random.randint(2) == 1)) # .25 probability of temporal extension
...
class TestTemporalExtensionManagerBlock():
    def reset(self):
        return
