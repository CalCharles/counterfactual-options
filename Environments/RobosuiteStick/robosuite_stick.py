...
class RoboStickEnvironment(RawEnvironment):
    def __init__(self, control_freq=2, horizon=30, renderable=False, standard_reward=-1, goal_reward=10, out_of_bounds_reward=-1):
        super().__init__()
        self.renderable = renderable
        self.env = robosuite.make(
            "StickPush",
        ...
        low, high = self.env.action_spec
        self.action_space = spaces.Box(low=np.array(low[:3].tolist() + [low[-1]]) , high=np.array(high[:3].tolist() + [high[-1]]))

        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=[9])
        self.num_actions = -1 # this must be defined, -1 for continuous. Only needed for primitive actions
        self.name = "RoboStick" # required for an environment 
        ...
        self.action_shape = (4,) # should be set in the environment, (1,) is for discrete action environments
        self.action = np.array([0,0,0,0])
        self.reward = 0
        self.done = False
        self.seed_counter = -1
        self.objects = ["Action", "Gripper", "Stick", "Block", "Target", "Reward", "Done", "Grasped"]
    ...
    def set_named_state(self, obs_dict):
        ...
        obs_dict['Grasped'] =  np.array([float(obs_dict['stick_grasped'])])
